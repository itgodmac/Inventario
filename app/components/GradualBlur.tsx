...
const PRESETS: Record<string, Partial<GradualBlurProps>> = {
    top: { position: 'top', height: '96px' },
    bottom: { position: 'bottom', height: '96px' },
    left: { position: 'left', height: '96px' },
    right: { position: 'right', height: '96px' },

    subtle: { height: '64px', strength: 1, opacity: 0.8, divCount: 3 },
    intense: { height: '160px', strength: 4, divCount: 8, exponential: true },

    smooth: { height: '128px', curve: 'bezier', divCount: 10 },
    sharp: { height: '80px', curve: 'linear', divCount: 4 },

    header: { position: 'top', height: '128px', curve: 'ease-out' },
    footer: { position: 'bottom', height: '128px', curve: 'ease-out' },
    sidebar: { position: 'left', height: '96px', strength: 2.5 },
    ...
    const blurDivs = useMemo(() => {
        ...
        for (let i = 1; i <= config.divCount; i++) {
            ...
            if (config.exponential) {
                blurValue = Number(math.pow(2, progress * 4)) * currentStrength;
            } else {
                blurValue = (progress * config.divCount + 1) * currentStrength;
            }
            ...
            const divStyle: CSSProperties = {
                ...
                backdropFilter: `blur(${blurValue.toFixed(0)}px)`,
